...
NOT_CHANGED: tuple[Exclusion, ...] = (
    ...
    Exclusion(
        "Dota 2: правка video.txt (уровни 2→3, mat_queue_mode, dxlevel) и «только чтение»",
        "cpu_level, gpu_level и mem_level — класс железа, который игра "
        "определяет сама; завышение сообщает ей, что ПК мощнее, и в лучшем "
        "случае ничего не даёт. mat_queue_mode и dxlevel — ключи движка "
        "Source 1, а Dota 2 с 2015 года на Source 2; в видео-источнике они "
        "ещё и записаны с опечатками («setting mindxlevel» без точки), то "
        "есть не совпадают ни с одним ключом. «Только чтение» запрещает "
        "игроку сохранить свои настройки графики. На клубном ПК правка к "
        "тому же бесполезна: файл лежит в userdata отдельно для каждого "
        "аккаунта, а ОЧИСТКА СТИМА удаляет userdata.",
        "placebo",
    ),
    Exclusion(
